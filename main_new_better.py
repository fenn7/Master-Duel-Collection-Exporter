...
previous_card_info = {}
# Global stop flag for GUI interruption
stop_flag = False
# Global print function for GUI redirection
print_func = print
...
def find_game_window(keyword: str = WINDOW_TITLE_KEYWORD) -> Optional[gw.Win32Window]:
    ...
    if not wins:
        print_func("Could not find game window. Please ensure it is open and not minimized.")
        return None
    win = wins[0]
    if win.isMinimized:
        try:
...
def grab_region(region: Tuple[int, int, int, int]) -> np.ndarray:
    ...
    try:
        ...
        if DEBUG:
            print_func(f"Screen capture failed for region {region}: {e}")
        raise
...
def detect_full_collection_area(win):
    ...
    try:
        ...
        if DEBUG:
            print_func(f"Failed to grab window region: {e}")
        return None, None
...
def prepare_csv_data(cards_in_order) -> List:
    ...
    print_func("Preparing CSV file data...")
    csv_data = []
    try:
...
def write_csv(csv_data, message=None):
    ...
    print_func("Saving collection to CSV file...")
    try:
        import csv
        ...
        print_func(final_message)
    except Exception as e:
        print_func(f"Failed to export CSV: {e}")

def print_card_summary(cards_in_order: List):
    """Print final card summary with color-coded output"""
    print_func("Finished processing cards. Preparing final summary...")
    LIGHT_BLUE = "\033[94m"
    BASIC_COLOR = "\033[97m"
    ...
    print_func(f"\n=== FINAL CARD SUMMARY ===")
    print_func(f"Found {len(cards_in_order)} unique card(s) in encounter order:")
    ...
    print_func(f"Total unique cards: {total_unique}")
    print_func(f"Total card count: {total_cards}")
    if gem_pack_cards:
        print_func(f"Gem Pack & Structure Deck: {len(gem_pack_cards)} unique cards")
    if legacy_pack_cards:
        print_func(f"Legacy Pack: {len(legacy_pack_cards)} unique cards")
    if CSV:
...
def signal_handler(sig, frame, cards_in_order_ref=None):
    ...
    print_func("\n\n=== SCRIPT INTERRUPTED ===")
    print_func("Processing was cancelled. Here is the summary of cards analyzed so far:")
    print_func("-" * 50)
    ...
    if SUMMARY:
        if cards:
            ...
            print_func("No cards were analyzed before interruption.")
    else:
        print_func("Summary printing disabled.")
    if CSV and cards:
...
def main(print_func_param=None):
    """Main entry point - process Master Duel collection"""
    global print_func
    if print_func_param is not None:
        print_func = print_func_param
    else:
        print_func = print
    print_func("Starting Master Duel Collection Exporter...")
    print_func("- Phase 1: Process first 4 rows without scrolling")
    print_func("- Phase 2: Process remaining rows with scrolling")
    print_func("Press Ctrl+C at any time to stop and see current results.")
    cards_container = [[]]
    signal.signal(signal.SIGINT, lambda sig, frame: signal_handler(sig, frame, cards_container))
    win = find_game_window()
    if not win:
        print_func("Could not find Master Duel window. Please ensure the game is open and visible.")
        return
    print_func("Game window found. Starting card detection and clicking process...")
    try:
        ...
        print_func("\n=== Process Complete ===")
        print_func("The collection scanner has finished processing all rows.")
